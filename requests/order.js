...
export function request_placeOrder(serverVersion, p) {
  ...
  if (serverVersion < ServerVersion.MIN_SERVER_VER_SCALE_ORDERS3) {
    if (order.scalePriceIncrement > 0 && order.scalePriceIncrement != null) {
      if (
        order.scalePriceAdjustValue != null ||
  ...
  if (serverVersion < ServerVersion.MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE && contract.secType == "BAG") {
  ...
  if (serverVersion < ServerVersion.MIN_SERVER_VER_AUTO_PRICE_FOR_HEDGE && order.dontUseAutoPriceForHedge) {
    throw new Error("It does not support dontUseAutoPriceForHedge parameter");
  }

  if (serverVersion < ServerVersion.MIN_SERVER_VER_ORDER_CONTAINER && order.isOmsContainer) {
    throw new Error("It does not support oms container parameter");
  }

  if (serverVersion < ServerVersion.MIN_SERVER_VER_PRICE_MGMT_ALGO && order.usePriceMgmtAlgo) {
  ...
  if (serverVersion >= ServerVersion.MIN_SERVER_VER_SMART_COMBO_ROUTING_PARAMS && contract.secType == "BAG") {
    let smartComboRoutingParamsCount = order.smartComboRoutingParams ? order.smartComboRoutingParams.length : 0;
  ...
  if (serverVersion >= ServerVersion.MIN_SERVER_VER_DELTA_NEUTRAL_CONID && order.deltaNeutralOrderType) {
  ...
  if (serverVersion >= ServerVersion.MIN_SERVER_VER_DELTA_NEUTRAL_OPEN_CLOSE && order.deltaNeutralOrderType) {
