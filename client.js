...
import { request_mktData, request_tickByTickData, request_cancelTickByTickData } from "./requests/market-data.js";
...
class Client {
  ...
  */
  ...
  async _connect() {
    ...
    let serverVersion = await this._incomeHandler.awaitMessageType(IncomeMessageType._SERVER_VERSION);
    ...
    if (this._marketDataType != null) {
      ...
      this._protocolBytes.sendFieldset([OutcomeMessageType.REQ_MARKET_DATA_TYPE, 1 /*VERSION */, this._marketDataType]);
    }
  }
  ...
  _connectSendStartApi() {
    ...
    this._protocolBytes.sendFieldset([START_API, VERSION, this._clientId, optCapab]);
  }
  ...
  async getCurrentTime() {
    ...
    await this._sendFieldsetExpirable([OutcomeMessageType.REQ_CURRENT_TIME, 1 /* VERSION */]);

    return await this._incomeHandler.awaitMessageType(IncomeMessageType.CURRENT_TIME);
  }
  ...
  async streamMarketData(p) {
    ...
    return this._incomeHandler.requestIdEmitter(p.requestId, () => {
      this._sendFieldsetRateLimited([OutcomeMessageType.CANCEL_MKT_DATA, 2 /* VERSION */, p.requestId]);
    });
  }
  ...
  async reqMarketDataType(marketDataType) {
    ...
    await this._sendFieldsetRateLimited([OutcomeMessageType.REQ_MARKET_DATA_TYPE, 1 /*VERSION */, marketDataType]);
  ...
  async listenOrder(order, stop = console.log) {
    return this._incomeHandler.requestIdEmitter(order, stop);
  }
  ...
  async streamTickByTickData(p) {
    ...
    */
    ...
    return this._incomeHandler.requestIdEmitter(p.requestId, () => {
      this._sendFieldsetRateLimited(request_cancelTickByTickData(this._serverVersion, p.requestId));
    });
  }
  ...
  */
  ...
  async cancelOrder(orderId) {
    ...
    await this._sendFieldsetExpirable([OutcomeMessageType.CANCEL_ORDER, 1 /* VERSION */, orderId]);
  ...
  async getOpenOrders() {
    ...
    await this._sendFieldsetExpirable([OutcomeMessageType.REQ_OPEN_ORDERS, 1 /* VERSION */]);

    return await this._incomeHandler.awaitMessageType(IncomeMessageType.OPEN_ORDER_END);
  }
  ...
  async reqAutoOpenOrders(bAutoBind) {
    ...
    await this._sendFieldsetRateLimited([OutcomeMessageType.REQ_AUTO_OPEN_ORDERS, 1 /* VERSION */, bAutoBind]);
  }
  ...
  async getAllOpenOrders() {
    ...
    await this._sendFieldsetExpirable([OutcomeMessageType.REQ_ALL_OPEN_ORDERS, 1 /* VERSION */]);
  ...
  async reqGlobalCancel() {
    ...
    await this._sendFieldsetRateLimited([OutcomeMessageType.REQ_GLOBAL_CANCEL, 1 /* VERSION */]);
  }
  ...
  */
  ...
  async cancelHistoricalData(requestId) {
    ...
    await this._sendFieldsetRateLimited([OutcomeMessageType.CANCEL_HISTORICAL_DATA, VERSION, requestId]);
  }
  ...
  */
  ...
  async reqScannerSubscription(p) {
    ...
    */
    ...
    if (this._serverVersion < ServerVersion.MIN_SERVER_VER_SCANNER_GENERIC_OPTS && cannerSubscriptionFilterOptions != null) {
